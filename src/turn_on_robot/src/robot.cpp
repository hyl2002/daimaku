...
{
  ros::init(argc, argv, "robot");//ROS初始化 并设置节点名称，可修改
  ROS_INFO("robot node has turned on ");//显示状态
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
 ...
 //Cmd_Vel_Sub = n.subscribe(smoother_cmd_vel, 100, &turn_on_robot::Cmd_Vel_Callback, this);//订阅smoother_cmd_vel话题并控制机器人//差速 
  ...
  try{
     ...
     ROS_ERROR_STREAM("robot can not open serial port,Please check the serial port cable! ");//如果try失败，打印错误信息
  }
  if(Stm32_Serial.isOpen()){
    ROS_INFO_STREAM("robot serial port opened");//开启成功
  }else{
  }
}
/**************************************
