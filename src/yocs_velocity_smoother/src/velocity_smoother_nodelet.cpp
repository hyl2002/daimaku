...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
    ...
    double v_x_deviation_lower_bound = last_cmd_vel.linear.x - decel_lim_v_ * period * period_buffer;
    double v_x_deviation_upper_bound = last_cmd_vel.linear.x + accel_lim_v_ * period * period_buffer;

    double v_y_deviation_lower_bound = last_cmd_vel.linear.y - decel_lim_v_ * period * period_buffer;
    double v_y_deviation_upper_bound = last_cmd_vel.linear.y + accel_lim_v_ * period * period_buffer;
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      double v_inc,max_v_inc,v_x_inc,v_y_inc, w_inc, max_v_x_inc,max_v_y_inc, max_w_inc;
      ...
      if ((robot_feedback == ODOMETRY) && (current_vel.linear.x*target_vel.linear.x < 0.0))
      {
        ...
        max_v_x_inc = decel_lim_v_*period;
      }
      else
      {
        max_v_x_inc = ((v_x_inc*target_vel.linear.x > 0.0)?accel_lim_v:decel_lim_v_)*period;
      }

      if ((robot_feedback == ODOMETRY) && (current_vel.linear.y*target_vel.linear.y < 0.0))
      {
        // countermarch (on robots with significant inertia; requires odometry feedback to be detected)
        max_v_y_inc = decel_lim_v_*period;
      }
      else
      {
        max_v_y_inc = ((v_y_inc*target_vel.linear.y > 0.0)?accel_lim_v:decel_lim_v_)*period;
      }
      ...
      v_inc = sqrt(    v_x_inc *     v_x_inc +     v_y_inc *     v_y_inc);
      max_v_inc = sqrt(    max_v_x_inc *     max_v_x_inc +     max_v_y_inc *     max_v_y_inc);
      ...
      if (std::abs(v_inc) > max_v_inc)
      {
        ...
        cmd_vel->linear.x  = last_cmd_vel.linear.x  + sign(v_x_inc)*max_v_inc;
      }

      if (std::abs(v_y_inc) > max_v_inc)
      {
        // we must limit linear velocity
        cmd_vel->linear.y  = last_cmd_vel.linear.y  + sign(v_y_inc)*max_v_inc;
      }
